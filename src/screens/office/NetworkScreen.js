...
const NetworkNode = ({ member, isRoot = false }) => {
  ...
  const loadChildren = useCallback(async () => {
    ...
    try {
      console.log("REDE DO MEMBRO:", member.id);
        ...
        .from("profiles")
        .select("id, full_name, status, id_dr, is_active, avatar_url")
      ...
      console.log("ERRO LOAD CHILDREN:", err);
      Alert.alert("Erro", err?.message || "Falha ao carregar rede.");
    } finally {
      setLoading(false);
    }
  }, [member?.id]);

  // Raiz: carrega sozinho ao montar
  useEffect(() => {
    if (isRoot && member?.id && !loaded) {
      loadChildren();
      setIsOpen(true);
    }
  }, [isRoot, member?.id, loaded, loadChildren]);
  ...
  const borderOk =
    String(member?.status || "").toUpperCase() === "ATIVO" ||
    ...
    <View style={styles.nodeContainer}>
      <View style={styles.nodeWrapper}>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.memberCard,
            { borderColor: borderOk ? PALETTE.success : PALETTE.error },
          ]}
        >
      ...
      {isRoot && loaded && !loading && children.length === 0 && (
        <Text style={styles.emptyHint}>
          Nenhum direto encontrado para este usuário.
        </Text>
      )}
    </View>
  );
    ...
    async function loadInitial() {
      try {
        ...
        if (user) {
              const { data, error } = await supabase
            .from("profiles")
            .select("id, full_name, status, id_dr, avatar_url, is_active, sponsor_id")
            .eq("id", user.id)
            .single();

          console.log("ROOT PROFILE:", data);
